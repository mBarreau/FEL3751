...
def log_likelihood(theta, t_obs, y_obs, sigma):
    ...
    for s in zip(y_pred, y_obs):
        loglike += norm.logpdf(s[1], loc=s[0], scale=sigma) + sum(
            [dist_prior.logpdf(theta[i], scale=scale) for i in range(len(theta))]
        )
    return loglike
...
samples = sampler.get_chain(discard=100, flat=True)  # Discard burn-in and flatten

# %% Plot the results
fig, axs = plt.subplots(ndim, sharex=True, figsize=(7, 6))
for i in range(ndim):
...
plt.legend()
fig.suptitle(
    rf"Posterior distributions using {dist_prior.name} prior with $\sigma={scale}$"
)
fig.tight_layout()
plt.show()
